...
const Header: React.FC = () => {
    ...
    // Classes dinâmicas
    const textPrimary = isScrolled ? "text-[#0A1931]" : "text-[#F8F9FA]";
    const logoPrimary = isScrolled ? "text-[#0A1931]" : "text-[#F8F9FA]";
        ...
        >
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
                ...
                <h1 className="text-3xl font-gold font-black tracking-widest">
                    <a
                        href="#home"
                        className={`transition-colors ${logoPrimary} hover:text-[#C8A97E]`}
                    >
                        ADV.<span className="text-4xl">G.R</span>
                        <span className="text-[#C8A97E]">AMON</span>
                    </a>
                </h1>
                ...
                <div className="hidden lg:flex space-x-8 items-center">
                    {SECTIONS.map((link) => (
                        <a
                            ...
                            className={`text-lg font-medium transition-all duration-300 relative group ${textPrimary} hover:text-[#C8A97E] ${
                        ...
                        >
                            {link.name}
                            <span
                                className={`absolute bottom-0 left-0 w-full h-[2px] bg-[#C8A97E] transform origin-left transition-transform duration-300 ${
                                    activeSection === link.href
                                        ? "scale-x-100"
                    ...
                    <a
                        href="#contato"
                        className="ml-4 px-6 py-2 bg-[#C8A97E] text-[#0A1931] font-bold rounded-full shadow-md hover:bg-yellow-700 transition-all transform hover:scale-105"
                    >
                        Consulta
                    </a>
                </div>
            ...
            >
                ...
                <nav className="flex flex-col items-center space-y-6 pb-10">
                    {SECTIONS.map((link) => (
                        <a
                            ...
                            className={`w-full text-center py-2 text-2xl font-semibold transition-colors rounded-md ${
                                activeSection === link.href
                                    ? "text-[#C8A97E] bg-gray-100"
                                    : "text-gray-800 hover:text-[#C8A97E] hover:bg-gray-100"
                            }`}
                        >
                            {link.name}
                        </a>
                    ...
                    <a
                        ...
                        className="mt-6 px-6 py-3 bg-[#C8A97E] text-[#0A1931] font-bold rounded-full shadow-md text-center hover:bg-yellow-700 transition-all flex items-center justify-center gap-1"
                    >
